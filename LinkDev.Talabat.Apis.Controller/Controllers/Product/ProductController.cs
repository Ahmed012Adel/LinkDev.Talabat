...
namespace LinkDev.Talabat.Apis.Controller.Controllers.Product
{
    public class ProductController(IServiceManager serviceManager) : ApiControllerBase
    {
        ...
        [HttpGet]
        public async Task<ActionResult<IEnumerable<ProductToReturnDto>>> GetProductsAsync(string? Sort)
        {
            var Product = await serviceManager.ProductService.GetProductsAsync(Sort);
            return Ok(Product);
        }
